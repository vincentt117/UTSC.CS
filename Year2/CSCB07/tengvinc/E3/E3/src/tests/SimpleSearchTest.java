...
public class SimpleSearchTest {
	...
	public void testSearch() throws IOException, ParseException {
		...
		assertTrue("Did not return integer index", SimpleSearch.search(oneList, "1") == (int)SimpleSearch.search(oneList, "1"));
		
		//Test with only one element
		int actualOne = SimpleSearch.search(oneList, "1");
		int expectOne = 0;
		assertEquals(String.format("Expected: %s, got: %s", expectOne, actualOne), expectOne, actualOne);
		
		//Test with multiple different elements, looking for the first
		int actualDif = SimpleSearch.search(difList, "1");
		int expectDif = 0;
		assertEquals(String.format("Expected: %s, got: %s", expectDif, actualDif), expectDif, actualDif);

		//Test with multiple different elements, looking for the first
		int actualDif1 = SimpleSearch.search(difList, "2");
		int expectDif1 = 1;
		assertEquals(String.format("Expected: %s, got: %s", expectDif1, actualDif1), expectDif1, actualDif1);
		
		//Test with multiple different elements, looking for the first
		int actualDif2 = SimpleSearch.search(difList, "3");
		int expectDif2 = 2;
		assertEquals(String.format("Expected: %s, got: %s", expectDif2, actualDif2), expectDif2, actualDif2);
		
		//Test with multiple identical elements
		int actualSame = SimpleSearch.search(sameList, "1");
		int expectSame = 0;
		assertEquals(String.format("Expected: %s, got: %s", expectSame, actualSame), expectSame, actualSame);
